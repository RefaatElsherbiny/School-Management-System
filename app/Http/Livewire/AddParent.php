...
use App\Exports\my_parent_Excel;
...
class AddParent extends Component
{
    ...
    public $currentStep = 1,
        ...
        {
            return Excel::download(new my_parent_Excel, 'users.xlsx');

        //    return Excel::download(new my_parent_Excel, 'users.xlsx');
        }
        public function pdf() {

            return (new my_parent_Excel)->download('PDF.pdf', \Maatwebsite\Excel\Excel::DOMPDF);
        ...
        public function xlsx ()
        {
            ...
            return (new my_parent_Excel)->download('XLSX.xlsx', \Maatwebsite\Excel\Excel::XLSX);

        }
        public function html ()
        {
            return (new my_parent_Excel)->download('html.html', \Maatwebsite\Excel\Excel::HTML);


        }
        public function csv ()
        {
            return (new my_parent_Excel)->download('csv.csv', \Maatwebsite\Excel\Excel::CSV);
...
// }
    ...
    {
        return view('admin.livewire.add-parent', [
            'Nationalities' => Type_nationalite::all(),
            'Type_Bloods' => Type_Blood::all(),
            'Religions' => Type_religion::all(),
            // 'my_parents' => My_Parent::paginate(10),

            'my_parents' => My_Parent::orderBy('Name_Father')->paginate($this->pagination),
            'my_parents' => My_Parent::where('Name_Father', 'like', '%'.$this->search.'%')
    ...
    public function edit($idg)
    {
        ...
        $My_Parent = My_Parent::where('id',$idg)->first();
        $this->Parent_id = $idg;
    ...
    public function submitForm_edit(){
        ...
        if ($this->Parent_id){
            ...
            $parent->update([
                ...
                // 'Name_Father' => $this -> Name_Father_en,
            ]);
...
//         try {
//             My_Parent::findOrFail($ids)->delete();
//             return redirect()->to('/add_Parent');        }
...
// Delete one
public function deleteConfirm($idf){
    $info = My_Parent::find($idf);
    $this->dispatchBrowserEvent('SwalConfirm',[
        'title'=>'Are you sure?',
        'html'=>'You want to delete <strong>'.$info->Name_Father.'</strong>',
        'id'=>$idf
    ]);
}

public function delete($id_delete_one){
    try {
        $del =  My_Parent::find($id_delete_one)->delete();
        if ($del) {
            $this->dispatchBrowserEvent('deleted');
        }

        $this->check_parent = [];
        session()->flash('message', 'yes delete one');
...
public function deleteCountries(){
    $this->dispatchBrowserEvent('swal:deleteCountries',[
        ...
        'checkedIDs'=>$this->check_parent,
    ]);
}

public function deleteCheckedCountries($id_delete_all){
    try {

    My_Parent::whereKey($id_delete_all)->delete();
    $this->check_parent = [];
    session()->flash('message', 'yes delete all');
