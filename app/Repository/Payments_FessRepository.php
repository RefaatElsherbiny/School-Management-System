...
use App\Students;
use App\Models\Payments_fess;

use Illuminate\Support\Facades\DB;

class Payments_FessRepository implements Payments_FeesRepositoryInterface
{

    public function index()
    {
        $payment_students = Payments_fess::all();
        return view('admin.Payment.index',compact('payment_students'));
    }

    public function show($id)
    {
        $student = Students::findorfail($id);
        return view('admin.Payment.add',compact('student'));
    }

    public function edit($id)
    {
        $payment_student = Payments_fess::findorfail($id);
        return view('admin.Payment.edit',compact('payment_student'));
    }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $payment_students = new Payments_fess();
    ...
    public function update($request)
    {
        ...
        try {
            ...
            $payment_students = Payments_fess::findorfail($request->id);
            $payment_students->date = date('Y-m-d');
            $payment_students->student_id = $request->student_id;
            $payment_students->amount = $request->Debit;
            $payment_students->description = $request->description;
            ...
            // حفظ البيانات في جدول الصندوق
            $fund_accounts = Found_account::where('payment_id',$request->id)->first();
            $fund_accounts->date = date('Y-m-d');
            $fund_accounts->payment_id = $payment_students->id;
            $fund_accounts->Debit = 0.00;
            $fund_accounts->credit = $request->Debit;
    ...
    {
        try {
            Payments_fess::destroy($request->id);
